import React, { useEffect, useState } from 'react';
import axios from "axios"
import { useNavigate } from 'react-router-dom';



const Pokemon = ({urlPokemon}) => {
    const [pokemonDetail,setPokemonDetail]=useState([])
    const navigate= useNavigate()

    useEffect(()=>{
        axios.get(urlPokemon)
        .then(res=> setPokemonDetail(res.data))
    },[])
    

    const BackgroundColor = ()=>  {

        if(pokemonDetail.types?.[0].type.name == "normal"){
            return "#E7E7E7"
        }else if(pokemonDetail.types?.[0].type.name == "fighting"){
            return "#07B1B1"
        }else if(pokemonDetail.types?.[0].type.name == "flying"){
            return "#A07E00"

        }else if(pokemonDetail.types?.[0].type.name == "poison"){
            return "#00C126"
        }else if(pokemonDetail.types?.[0].type.name == "ground"){
            return "#7A520C"
        }else if(pokemonDetail.types?.[0].type.name == "rock"){
            return "#382607"
        }else if(pokemonDetail.types?.[0].type.name == "ghost"){
            return "#3C0058"
        }else if(pokemonDetail.types?.[0].type.name == "steel"){
            return "#B8B6B9"
        }else if(pokemonDetail.types?.[0].type.name == "fire"){
            return "#FD8080"
        }else if(pokemonDetail.types?.[0].type.name == "water"){
            return "#80CCFD"
        }else if(pokemonDetail.types?.[0].type.name == "grass"){
                return "#B1FD80"
        }else if(pokemonDetail.types?.[0].type.name == "electric"){
                return "#FBE035"
        }else if(pokemonDetail.types?.[0].type.name == "psychic"){
                return "#CCD3F7"
        }else if(pokemonDetail.types?.[0].type.name == "ice"){
                return "#B1FD80"
        }else if(pokemonDetail.types?.[0].type.name == "dragon"){
                return "#FFAE00"
        }else if(pokemonDetail.types?.[0].type.name == "dark"){
                return "#000000"
        }else if(pokemonDetail.types?.[0].type.name == "fairy"){
                return "#F6F6F6"
        }else if(pokemonDetail.types?.[0].type.name == "shadow"){
                return "#A9A5A5"
        }else{
            return "#C7C9D3"
        }
           
              
    }

    return (
        <div className='containerPokemon' style={{backgroundColor:`${BackgroundColor()}`}} onClick={()=>navigate(`/pokedex/${pokemonDetail.id}`)}>
            
            <h2>{pokemonDetail.name}</h2>
            <div className='contentImg'>
               
                <img src={pokemonDetail.sprites?.other.dream_world.front_default} alt="" />
            </div>

            <div className='containerAttributes' >

                <p>Type: {pokemonDetail.types?.[0].type.name}</p>
                <p>Hp: {pokemonDetail.stats?.[0].base_stat}</p>
                <p>Attack: {pokemonDetail.stats?.[1].base_stat}</p>
                <p>Defense: {pokemonDetail.stats?.[2].base_stat}</p>
                <p>Speed: {pokemonDetail.stats?.[3].base_stat}</p>
            </div>
        </div>
    );
};

export default Pokemon;